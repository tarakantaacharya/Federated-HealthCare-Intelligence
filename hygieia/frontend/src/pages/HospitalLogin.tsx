...
const HospitalLogin: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await apiClient.loginHospital({ username, password });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-4 py-12">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8">
        <h2 className="text-3xl font-bold text-center text-slate-900">Hospital Login</h2>
        <p className="mt-2 text-center text-sm text-slate-600">
          Enter your credentials to access your hospital node dashboard.
        </p>
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div>
            ...
            <input
              ...
              required
              className="appearance-none relative block w-full px-3 py-2 border border-slate-300 rounded-lg placeholder-slate-500 text-slate-900 focus:outline-none focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm"
